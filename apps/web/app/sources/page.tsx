...
export default async function SourcesPage() {
  const [sources, catalog, metrics, quality] = await Promise.all([getSources(), getMetricCatalog(true), getMetrics(), getQuality()]);
  const unavailable = catalog.filter((metric) => metric.source_status !== "available");
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Sources</h1>
        <p className="mt-1 text-sm text-muted">Method, source files, metric definitions, and latest data-quality checks.</p>
      </div>
            ...
            {catalog.map((metric) => (
              <tr className="border-t border-line align-top" key={metric.catalog_item_id}>
                ...
                <td className="px-3 py-2">
                  <span className={`rounded-md px-2 py-1 text-xs font-medium ${metric.source_status === "available" ? "bg-emerald-50 text-teal" : "bg-amber-50 text-amber"}`}>
      ...
      {unavailable.length > 0 ? (
        <section className="panel p-4">
          ...
          <p className="mt-1 text-sm text-muted">These are present in the reference taxonomy but not currently available as live source-backed or calculated metrics.</p>
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            {unavailable.map((metric) => (
              <div className="rounded-md border border-line p-3" key={`missing-${metric.catalog_item_id}`}>
                <div className="text-sm font-medium">{metric.metric_name}</div>
                <div className="mt-1 text-xs uppercase text-muted">{metric.catalog_group} · {metric.source_status.replace("_", " ")}</div>
              </div>
            ))}
          </div>
        </section>
            ...
            {metrics.map((metric) => (
              <tr className="border-t border-line align-top" key={metric.metric_id}>
                ...
                <td className="px-3 py-2 text-muted">
                  {metric.definition}
                  {metric.calculation_method ? <span className="block pt-1 text-amber">Calculation: {metric.calculation_method}</span> : null}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <section className="panel overflow-hidden">
        <table className="w-full border-collapse text-left text-sm">
          ...
          <tbody>
            {quality.map((check) => (
              <tr className="border-t border-line align-top" key={`${check.run_id}-${check.check_name}-${check.source_file_id}`}>
                <td className="px-3 py-2 font-medium">{check.status}</td>
                <td className="px-3 py-2">{check.check_name}</td>
                <td className="px-3 py-2 text-muted">{check.source_name ?? check.source_file_id}</td>
