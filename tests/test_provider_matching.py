...
def test_normalize_provider_name_removes_punctuation_and_leading_the() -> None:
    assert normalize_provider_name("The University of Technology, Sydney") == (
        "university of technology sydney"
    )
    assert normalize_provider_name("University of New South Wales(1.03)") == (
        "university of new south wales"
    )


def test_resolves_source_aliases() -> None:
    assert resolver().resolve("The University of Melbourne").provider_id == (
        "university_of_melbourne"
    )
    assert resolver().resolve("University of Technology, Sydney").provider_id == (
